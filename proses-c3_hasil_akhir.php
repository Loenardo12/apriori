...
<a style="padding:3px 6px 3px 6px; border:1px solid #06F;"  href="hal_admin.php?page=c2tes">tes c2</a>
<?php
include "koneksi.php";
?>
...
<tr style="background:linear-gradient(to top, #CCC, #999 );">
	...
	<?php
		...
		<tr>
			...
			?></td>
			<td><?php echo $rowC3['support_count'];?></td>
			<td><?php echo substr($rowC3['persen_support'],0,5);?></td>
		</tr>
		<?php }
		?>
		<tr>
...
<table width="700" border="1" align="left" cellpadding="0" cellspacing="2" style="border:thin; border-color:#399; padding-left:3px; margin-top:5px;">
<tr style="background:linear-gradient(to top, #CCC, #999 );">
  <td colspan="6"><strong>Hasil Pembentukan Item C3 (3 itemset) Dengan Minimum Support adalah = 
    <?php
  $strRule=mysqli_query("SELECT * FROM rule WHERE kdrule='R3' ");
		$dataRule=mysqli_fetch_array($strRule); $nRule=$dataRule['minsupport']; 
		echo "<a href='edit-rule.php?kdrule=R3'>$nRule %</a>";
  ?></strong></td>
  </tr>
<tr style="background:linear-gradient(to top, #CCC, #999 );">
			<td width="380">Itemset</td>
	...
	<?php
		//kosong tmp transaksi
 		...
 		$resultC3=mysqli_query("SELECT * FROM itemc3 WHERE persen_support>='$nRule' ORDER BY kditem1,kditem2 ASC ");
		$support_count="0";
		while($rowC3 = mysqli_fetch_array($resultC3))
		{
		?>
...
<p>Pembentukan aturan assosiatif cukup penting untuk mendapatkan dan menghitung nilai confidence. perlu diketahui algoritma apriori cukup boros dalam penggunaan memory dan paling banyak menghabiskan waktu saat scanning.</p>
</div>
</body>
</html>